...
int main(int, char*[])
{
  ...
  /**
   * @brief element buffer objects (EBO)
   *
   * if we want to draw a rectangle instead of a triangle, one possibility would
   * be:
   * float vertices[] = {
   *     // first triangle
   *      0.5f,  0.5f, 0.0f,  // top right
   ...
   *     -0.5f,  0.5f, 0.0f   // top left
   * };
   * however, this implementation would lead to 50% overhead, since same
   * rectangle could be specified only with 4 vertices
   *
   * **EBO**: stores indices that OpenGL uses to decide what vertices to draw
   * !!!INDEXED DRAWING!!! (seems almost as STL format...)
   */
  ...
  /**
   * @brief bind a buffer object to the CURRENT BUFFER TYPE TARGET and copy the
   *        indices data into the buffer
   *
   * (in a similar fashion to VBO)
   */
